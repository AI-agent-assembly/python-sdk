...
async def test_terminal_pending_blocks_tool_and_does_not_run(
    ...
    tool = function_tool_cls(name="terminal_pending_tool")
    ctx = SimpleNamespace(agent_id="agent-terminal-pending")
    result = await tool.on_invoke_tool(ctx, '{"q": "secret"}')

    # A string governance error (not the tool's real dict output) proves the
    # body never executed under a terminal "pending".
    assert isinstance(result, str)
    assert "Approval denied for tool 'terminal_pending_tool'" in result
