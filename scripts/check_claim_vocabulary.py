...
python-sdk's reader-facing Markdown. AAASM-5798 / AAASM-5599.

Adapted from agent-assembly's `scripts/check_claim_vocabulary.py` (AAASM-5679)
and docs Hub's port (AAASM-5797). Per AAASM-5795's federation-by-convention
decision this is a ONE-TIME PORT, not a shared dependency -- this file
diverges independently from the two it was ported from. What is kept
verbatim is kept because the underlying problem is identical (Markdown
source, the same ADR 0033 forbidden-design-7 absolutes, the same
false-positive shape from quoting a banned phrase to forbid it), not because
of a code-sharing obligation.

File scope differs from both prior ports: `quickstart_snippets/` is excluded
(ruff.toml already excludes it for the same reason -- vendored, verbatim
copies from the `examples` repo, regenerated by `generate_quickstart_tabs.py`
into `docs/quick-start.md`; this repo does not author that prose and
flagging it would point an author at text they cannot fix here) and so is
`verification-reports/**` (its purpose is to quote overstatements in order
to disprove them, same as agent-assembly's own exclusion).
...
EXCLUDE_GLOBS = (
    # Vendored, verbatim copies from the `examples` repo -- this repo does
    # not author this prose. ruff.toml excludes the same directory for the
    # same reason.
    "quickstart_snippets/**",
...
def _opens_front_matter(lines: list[str]) -> bool:
    ...
    break. This repo's Markdown (unlike docs Hub's mdBook corpus) may carry
    front matter, so -- unlike the docs Hub port -- this check is real, not
    asserted away."""
...
def _structural_ranges(text: str) -> list[tuple[int, int]]:
    ...
    `<meta …>` tag -- each is quoted alone somewhere a reader never sees the
    surrounding sentence's exemption label."""
...
def scan_text(path: str, original: str) -> list[Diagnostic]:
    ...
    for rule, compiled in COMPILED:
        for m in compiled.finditer(match_text):
            ...
            rel = m.start() - start
            in_quote = any(lo <= rel < hi for lo, hi in _quoted_spans(match_text[start:end])) and not _in_ranges(
                structural, m.start()
            )
            if in_quote:
                diagnostics.append(
...
SELFTEST_CASES: tuple[tuple[str, str | None], ...] = (
    ...
    ("There is no claim of complete detection.", None),  # NEG guard
    ("catch-all rules are configured here", None),  # CFG-NOUN guard
    ("a catch-all promise", "CLAIM-ABS-02"),
    ("`immutable audit` is a banned phrase", None),  # inline code
...
def selftest() -> int:
    ...
    if in_scope("quickstart_snippets/langchain/basic.py"):
        failures.append("in_scope() accepts a vendored quickstart snippet")
    if in_scope("verification-reports/AAASM-2714.md"):
        failures.append("in_scope() accepts a verification report")
    if in_scope("docs/index.txt"):
